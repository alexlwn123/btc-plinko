...
export type Risk = keyof typeof RISKS;
...
export type ProvablyFairDrop = Drop & {
  fairness: {
    serverSeed: string;
...
export const STAKE_MULTIPLIERS: Record<number, Record<Risk, number[]>> = {
  8: {
...
export function assertRows(rows: number) {
...
export async function buildProvablyFairDrop(
  rows: number,
  {
    serverSeed,
    serverSeedHash,
  ...
  const points: DropPoint[] = [{ layer: 0, rights: 0, slot: 0 }];
  let rights = 0;
...
export async function verifyProvablyFairDrop(drop: ProvablyFairDrop) {
...
export function buildDrop(rows: number, bitSource = unbiasedBit): Drop {
  assertRows(rows);
...
export function buildMultipliers(rows: number, riskName: string) {
  assertRows(rows);

  const risk = riskName in RISKS ? (riskName as Risk) : "medium";
